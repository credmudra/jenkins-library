                ...
                steps{
                    ...
                    script {
                        if (env.BRANCH_NAME =~ /^(develop|(feature|(bug|hot)fix)(\/[a-zA-Z0-9]+([-_][a-zA-Z0-9]+)*){1,2}|\/[0-9]+(\.[0-9]+)*(-(alpha|beta|rc)[0-9]*)?)$/) {
                            ...
                            sh 'cp '+"$CREDMUDRA_ENV_BASE_DIRECTORY"+'/dev/.env .env.local'    
                        }
                         if (env.BRANCH_NAME == 'uat') {
                              sh 'cp '+"$CREDMUDRA_ENV_BASE_DIRECTORY"+'/uat/.env .'
                              sh 'cp '+"$CREDMUDRA_ENV_BASE_DIRECTORY"+'/uat/.env .env.local'
                        }
                
                        if (env.BRANCH_NAME == 'master') {
                            sh 'cp '+"$CREDMUDRA_ENV_BASE_DIRECTORY"+'/prod/.env .'
                            sh 'cp '+"$CREDMUDRA_ENV_BASE_DIRECTORY"+'/prod/.env .env.local'
                        }
                    }
                }
            }
                ...
                steps{
                    ...
                    script{
                        ...
                        if (env.BRANCH_NAME == 'master') {
                            ...
                            else if(projectName == 'cred-website' || projectName == 'credmudra-app'){
