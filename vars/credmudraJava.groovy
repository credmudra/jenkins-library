                ...
                steps{
                    ...
                    script{
                        ...
                        if (env.BRANCH_NAME =~ /^(develop|(feature|(bug|hot)fix)(\/[a-zA-Z0-9]+([-_][a-zA-Z0-9]+)*){1,2}|\/[0-9]+(\.[0-9]+)*(-(alpha|beta|rc)[0-9]*)?)$/) {
                            ...
                            sshagent(credentials: ['credmudra-dev-private-key']) {
                                ...
                                sh 'ssh -o StrictHostKeyChecking=no root@'+"$CREDMUDRA_DEV_SERVER_IP"+' docker run -d --network=\\"cred-servers\\" -p '+"$port"+':80 '+  "$volume"+ ' --env-file=.env --name '+"$projectName $baseName/$projectName"
                        ...
                        if (env.BRANCH_NAME == 'uat') {
                               sshagent(credentials: ['credmudra-uat-private-key']) {
                                ...
                                sh 'ssh -o StrictHostKeyChecking=no root@'+"$CREDMUDRA_UAT_SERVER_IP"+' docker run -d --network=\\"cred-servers\\" -p '+"$port"+':80 '+"$volume"+' --env-file=.env --name '+"$projectName $baseName/$projectName"
